...
namespace SoulsBetterDLC.NPCS.Bosses.ChampionofExaltation
{
    ...
    public class AuricSoul : ModNPC
    {
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
        {
            ...
            for (int i = 0; i < 7; i++)
            {
                spriteBatch.Draw(texture.Value, NPC.oldPos[i] + NPC.Size / 2 - Main.screenPosition, null, new Color(drawColor.R, drawColor.G, drawColor.B, (int)(200f / ((float)i+1f))) * (1-(i/ 7f)), NPC.rotation, texture.Size()/2, NPC.scale, SpriteEffects.None, 0);
            }
            return true;
        }
