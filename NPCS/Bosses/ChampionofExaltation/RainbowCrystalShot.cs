...
namespace SoulsBetterDLC.NPCS.Bosses.ChampionofExaltation
{
    ...
    public class RainbowCrystalShot : ModProjectile
    {
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Microsoft.Xna.Framework.Color color32 = Lighting.GetColor((int)((double)Projectile.position.X + (double)Projectile.width * 0.5) / 16, (int)(((double)Projectile.position.Y + (double)Projectile.height * 0.5) / 16.0));
            Vector2 vector74 = Projectile.position + new Vector2((float)Projectile.width, (float)Projectile.height) / 2f + Vector2.UnitY * Projectile.gfxOffY - Main.screenPosition;
            Texture2D texture2D4 = TextureAssets.Projectile[Projectile.type].Value;
            Microsoft.Xna.Framework.Rectangle rectangle31 = texture2D4.Frame(1, Main.projFrames[Projectile.type], 0, Projectile.frame, 0, 0);
            Microsoft.Xna.Framework.Color color94 = Projectile.GetAlpha(color32);
            Vector2 origin24 = rectangle31.Size() / 2f;
            Microsoft.Xna.Framework.Color color98 = Main.hslToRgb(Projectile.ai[0], 1f, 0.5f, byte.MaxValue).MultiplyRGBA(new Microsoft.Xna.Framework.Color(255, 255, 255, 0));
            ...
            if (Projectile.ai[1] != -1f && Projectile.Opacity > 0.3f)
            {
                ...
                Vector2 vector77 = new Vector2(1f, vector76.Length() / (float)texture2D4.Height);
        ...
        public override void AI()
        {
            ...
            Projectile.rotation = Projectile.localAI[1] / 40f * 6.2831855f * (float)Projectile.direction;
            ...
            for (int num877 = 0; num877 < 2; num877 = num3 + 1)
            {
                ...
                {
                    Vector2 value52 = Vector2.UnitY.RotatedBy((double)((float)num877 * 3.1415927f), default(Vector2)).RotatedBy((double)Projectile.rotation, default(Vector2));
            ...
            for (int num878 = 0; num878 < 2; num878 = num3 + 1)
            {
                if (Main.rand.NextBool(10))
                {
                    Vector2 value53 = Vector2.UnitY.RotatedBy((double)((float)num878 * 3.1415927f), default(Vector2));
        ...
        public void DoRainbowCrystalStaffExplosion()
        {
            ...
            for (float num2 = 0f; num2 < num; num2 += 1f)
            {
                ...
                Main.dust[num3].velocity = spinningpoint.RotatedBy((double)(6.2831855f * num2 / num), default(Vector2)) * value * (0.8f + Main.rand.NextFloat() * 0.4f);
            ...
            for (float num4 = 0f; num4 < num; num4 += 1f)
            {
                ...
                Main.dust[num5].velocity = spinningpoint.RotatedBy((double)(6.2831855f * num4 / num), default(Vector2)) * value * (0.8f + Main.rand.NextFloat() * 0.4f);
