...
namespace SoulsBetterDLC.NPCS.Bosses.ChampionofDevastation
{
    ...
    public class PlantHook : ModProjectile
    {
        ...
        {
            Player target = Main.player[(int)Main.npc[(int)Projectile.ai[0]].target];
        ...
        {
            NPC owner = Main.npc[(int)Projectile.ai[0]];
            Vector2 pos = new Vector2(Projectile.position.X + (float)(Projectile.width / 2), Projectile.position.Y + (float)(Projectile.height / 2));
            ...
            while (flag3)
            {
                ...
                Microsoft.Xna.Framework.Color color = Lighting.GetColor((int)pos.X / 16, (int)(pos.Y / 16f));
                Main.spriteBatch.Draw(TextureAssets.Chain26.Value, new Vector2(pos.X - Main.screenPosition.X, pos.Y - Main.screenPosition.Y), new Microsoft.Xna.Framework.Rectangle?(new Microsoft.Xna.Framework.Rectangle(0, 0, TextureAssets.Chain26.Width(), num8)), color, rotation, new Vector2((float)TextureAssets.Chain26.Width() * 0.5f, (float)TextureAssets.Chain26.Height() * 0.5f), 1f, SpriteEffects.None, 0f);
            }
            return true;
        }
