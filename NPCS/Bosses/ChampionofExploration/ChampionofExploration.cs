        ...
        public void WulfrumAttack()
        {
            ...
            if (attackTimer < sweepingTime && attackTimer > 60)
            {
                ...
                if (Main.netMode != NetmodeID.MultiplayerClient)
                {
                    ...
                    for (int i = 0; i < 2; i++)
                    {
                        Projectile scrap = Projectile.NewProjectileDirect(NPC.GetSource_FromAI(), NPC.Center, vel.RotatedBy(MathHelper.ToRadians(Main.rand.Next(-20, 20))) * (float)Main.rand.Next(1, 40) / 4f, ModContent.ProjectileType<WulfrumScrap>(), FargoSoulsUtil.ScaledProjectileDamage(210), 0);
